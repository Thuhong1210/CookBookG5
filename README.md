...
## Project Layout (2025 refresh)
To keep backend logic and frontend assets separated without changing behaviour or UI, the repository is now organised as follows:

- `backend/`: Flask application (`mycookbook` package), database scripts, requirements and Procfile. Run-time entry points live here (`run.py`, `run_app.py`).
- `frontend/`: All HTML templates, CSS, JavaScript, images and upload folders served by the Flask app.

### Running locally
```bash
python3 -m venv venv && source venv/bin/activate
pip install -r requirements.txt          # delegates to backend/requirements.txt
python run_app.py                        # automatically boots backend/run_app.py
# or:
cd backend && python run_app.py
```

Static uploads are stored in `frontend/static/uploads` (automatically created on boot), so the visual layer stays in the `frontend` tree while Python code remains in `backend`.
